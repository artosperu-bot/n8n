...
function containsNumber(text: string, value: number): boolean {
  return normalizeNumericText(text).includes(String(value));
}
...
export function evaluateHard(turn: QaTurn, observation: QaTurnObservation): QaFinding[] {
  ...
  for (const [field, code, actual, wanted] of checks) {
    if (hasOwn(expected, field) && actual !== wanted) {
      findings.push({ level: 'RED', code, message: `${field}: esperado=${String(wanted)} actual=${String(actual)}` });
    }
  }

  for (const phrase of expected.answerIncludes ?? []) {
    if (!answer.toLocaleLowerCase('es').includes(phrase.toLocaleLowerCase('es'))) {
      findings.push({ level: 'RED', code: 'ANSWER_REQUIRED_EVIDENCE_MISSING', message: `Falta en respuesta: ${phrase}` });
    }
  }
  for (const phrase of expected.answerExcludes ?? []) {
    if (answer.toLocaleLowerCase('es').includes(phrase.toLocaleLowerCase('es'))) {
      findings.push({ level: 'RED', code: 'ANSWER_FORBIDDEN_CONTENT', message: `Contenido prohibido presente: ${phrase}` });
    }
  }

  const erp = debug.erp ?? null;
  if (debug.intent === 'PRICE') {
    if (erp?.price != null && !containsNumber(answer, Number(erp.price))) {
      findings.push({ level: 'RED', code: 'PRICE_EVIDENCE_MISMATCH', message: `La respuesta no contiene el precio ERP ${erp.price}.` });
    }
    if (!erp && /S\/\s*[\d.,]+/i.test(answer)) {
      findings.push({ level: 'RED', code: 'UNSUPPORTED_NUMERIC_CLAIM', message: 'Afirmó un precio numérico sin evidencia ERP.' });
    }
  }
  if (debug.intent === 'STOCK') {
    if (erp?.stock != null && !containsNumber(answer, Number(erp.stock))) {
      findings.push({ level: 'RED', code: 'STOCK_EVIDENCE_MISMATCH', message: `La respuesta no contiene el stock ERP ${erp.stock}.` });
    }
    if (!erp && /\b\d+\s*(?:unidad|unidades)\b/i.test(answer)) {
      findings.push({ level: 'RED', code: 'UNSUPPORTED_NUMERIC_CLAIM', message: 'Afirmó stock numérico sin evidencia ERP.' });
    }
  }
