...
function sameProduct(a:unknown,b:unknown):boolean{return Boolean(a&&b&&String(a).trim().toLocaleLowerCase('es')===String(b).trim().toLocaleLowerCase('es'));}
...
export function assessCommercialContinuity(observation:QaTurnObservation):boolean{
  ...
  if(state.recommendationChanged===true){
    const from=String(state.recommendationChangeFrom??'').trim();const to=String(state.recommendedProduct??'').trim();const reason=String(state.recommendationChangeReason??'').trim();const communicated=state.recommendationChangeCommunicated===true;const visible=String(state.customerVisibleRecommendedProduct??'').trim();const text=answer.toLocaleLowerCase('es');const explicitChange=/\b(?:cambio|cambia|cambiar|cambiar[ií]a|ahora (?:te )?recomiendo|nueva recomendaci[oó]n)\b/i.test(answer);
  ...
  if(/\b(?:ese|esa|ese modelo|esa opci[oó]n)\b/i.test(observation.request.message)){
    const visible=state.selectedProduct??(!recommendationMismatch?state.salientProduct:null)??visibleRecommendation??state.salientProduct??null;const target=debug.queryTarget??state.queryTarget??state.activeProduct??null;if(visible&&target&&!sameProduct(visible,target))return false;
  }
  return true;
}
...
export function assessNba(observation:QaTurnObservation):QaNbaEvaluation{
  ...
  const repeatsKnown=nba==='ASK_MISSING_FACT'&&repeatsKnownQuestion(answer,state);const n1Delivered=actionDelivered(nba,answer,intent,debug)&&!repeatsKnown;const deliveryPass=n1Required?n1Delivered:Boolean(answer.trim());const actionabilityPass=!unsupportedCommercialAction(answer,nba,intent);const continuityPass=assessCommercialContinuity(observation);
...
function hasExplicitNeedCue(message:string):boolean{const text=foldText(message);return /\b(?:necesito|requiero|busco|priorizo|mi prioridad|me importa|lo mas importante|si o si|quiero que sea|necesito que)\b/.test(text);}
function isFocusedFactualIntent(intent:string):boolean{return ['ATTRIBUTE','CAPABILITY','PRICE_AVAILABILITY','PRICE','STOCK','IMAGES','IMAGE','POLICY','WARRANTY','ORDER_STATUS'].includes(intent);}
...
export function assessSpinUtility(observation:QaTurnObservation):boolean{
  const response=observation.response??{};const state=response.state??{};const debug=response.debug??{};const answer=String(response.answer??'');
  const action=String(state.lastNba??'').toUpperCase();const intent=String(debug.intent??state.lastIntent??'').toUpperCase();const message=String(observation.request.message??'');const contribution=String(state.lastSpinContribution??'').toUpperCase();
  ...
  const missing=String(state.pendingMissingFact??'');if(!missing||repeatsKnownQuestion(answer,state))return false;if((answer.match(/\?/g)??[]).length!==1)return false;
  const foldedMissing=foldText(missing);const consumable=/uso|problema|impacto|implicacion|consecuencia|prioridad|criterio|presupuesto|tope|modelo|producto/.test(foldedMissing);if(!consumable||!spinQuestionMatchesMissing(answer,missing))return false;
  if(/uso/.test(foldedMissing)&&(state.useCase||state.sector))return false;if(/problema/.test(foldedMissing)&&state.problem)return false;if(/impacto|implicacion|consecuencia/.test(foldedMissing)&&contribution==='IMPLICACION')return false;if(/prioridad|criterio/.test(foldedMissing)&&(state.priorities?.length??0)>0)return false;
  if(/presupuesto|tope/.test(foldedMissing)){if(state.budget!=null)return false;if(!['BUDGET_CONSTRAINT','OBJECTION','HANDLE_PRICE_OBJECTION'].includes(intent))return false;}
...
export function assessFabGrounding(observation:QaTurnObservation):boolean{
  const response=observation.response??{};const state=response.state??{};const debug=response.debug??{};const answer=String(response.answer??'');
  const attributes=Array.isArray(state.currentAttributes)?state.currentAttributes.map((x:unknown)=>String(x).toUpperCase()):[];const attributeText=attributes.join(' ');const technicalAttribute=/RAM|MEMORIA|BATERIA|RESISTENCIA|CAIDA|DURABILIDAD|CAMARA|TERMICA|5G|PANTALLA|NFC|CONECTIVIDAD|FISICO|PESO|DIMENSION|GROSOR/.test(attributeText);const decisionContext=['COMPARE','RECOMMEND','RECOMMEND_WITHIN_BUDGET'].includes(String(debug.intent??'').toUpperCase())||/^RAG_(?:COMPARISON|RECOMMENDATION)/.test(String(debug.route??''));
  ...
  const commercial=foldText(answer);
  const benefit=/\b(?:te ayuda|te da|mas margen|punto a favor|util|permite|facilita|reduce|reducir|evita|evitar|aguanta|aguantar|protege|proteger|riesgo|reparacion|reparaciones|conviene|ideal|encaja|adecuad[oa]|proteccion ante|mayor|superior|mas rapid[oa]|en tu caso|para (?:tu|ese|esa|trabajo|obra|construccion|uso|jornada)|si priorizas|si prefieres|para lo que)\b/i.test(commercial);
...
export function evaluateCommercial(observation:QaTurnObservation):QaFinding[]{
  ...
  const lengthGuidance=debug.intent==='COMPARE'?750:500;if(answer.length>lengthGuidance)findings.push({level:'YELLOW',code:'CHAT_TOO_LONG',message:`Respuesta de ${answer.length} caracteres; excesiva para chat comercial.`});
  ...
  const internalLanguage=/como modelo de ia|según mi sistema interno|\bINTENT\b|queryTarget|\bRAG\b|\bUNKNOWN\b|\boracle\b|\bconfidence\b|\bscore\b|datos recuperados|ficha técnica|según (?:la )?fuente(?: consultada)?|fuente (?:consultada|disponible)|evidencia (?:disponible|recuperada|técnica)|\bSOFT_CLOSE_TO_STOCK\b|\bASK_(?:USE_CASE|PROBLEM|IMPLICATION|PRIORITY|BUDGET)\b/i;if(internalLanguage.test(answer))findings.push({level:'YELLOW',code:'ROBOTIC_META_LANGUAGE',message:'Expone lenguaje técnico/meta o suena como sistema.'});
  const isPriceObjection=debug.priceObjection===true||debug.intent==='HANDLE_PRICE_OBJECTION';if(isPriceObjection&&!/te resulta alto|se sale de tu presupuesto|busquemos una opción|veamos una opción|claro|sí, el precio|podemos buscar/i.test(answer))findings.push({level:'YELLOW',code:'EMPATHY_WEAK_PRICE_OBJECTION',message:'La objeción de precio no se reconoce antes de avanzar.'});if(isPriceObjection&&/entiendo la objecion/i.test(foldText(answer)))findings.push({level:'YELLOW',code:'LAER_ROBOTIC_ACKNOWLEDGEMENT',message:'LAER reconoce la objeción con lenguaje de evaluador en vez de lenguaje humano.',rootCause:'WRITER'});
  const nba=assessNba(observation);const action=String(state.lastNba??'').toUpperCase();
  if(!action)findings.push({level:'YELLOW',code:'NBA_MISSING',message:'No se registró siguiente mejor acción para el turno.',rootCause:'NBA'});else if(!nba.decisionPass)findings.push({level:state.purchaseSignal===true?'RED':'YELLOW',code:action==='ANSWER_ONLY'&&nba.n1Required?'NBA_PROGRESSION_MISSING':'NBA_STAGE_MISMATCH',message:`La acción ${action} no progresa de forma compatible con ${nba.n1Reason}.`,rootCause:'NBA'});
  if(nba.n1Required&&!nba.n1Delivered&&action!=='ANSWER_ONLY')findings.push({level:'YELLOW',code:'NBA_NOT_DELIVERED',message:`La respuesta no ejecuta de forma visible ${action}.`,rootCause:'NBA'});if(action==='ASK_MISSING_FACT'&&repeatsKnownQuestion(answer,state))findings.push({level:'YELLOW',code:'NBA_REPEATS_KNOWN',message:'La siguiente pregunta solicita contexto que ya estaba disponible.',rootCause:'NBA'});if(!nba.actionabilityPass)findings.push({level:'RED',code:'UNSUPPORTED_COMMERCIAL_ACTION',message:'La respuesta ofrece una acción que no está autorizada por el N+1 ejecutable.',rootCause:'NBA'});if(!nba.continuityPass)findings.push({level:'RED',code:'COMMERCIAL_PRODUCT_SWITCH_UNEXPLAINED',message:'La decisión comercial cambió de producto o etapa sin una transición visible válida.',rootCause:'NBA'});if(!assessSpinUtility(observation))findings.push({level:'YELLOW',code:'SPIN_UTILITY_INVALID',message:'SPIN creó memoria indebida o la pregunta no corresponde al único dato faltante.',rootCause:'NBA'});if(!assessFabGrounding(observation))findings.push({level:'YELLOW',code:'FAB_GROUNDING_MISSING',message:'Un atributo verificado no se convirtió en un beneficio comercial seguro.',rootCause:'WRITER'});
  const message=observation.request.message.toLocaleLowerCase('es');if(/construcci[oó]n|se me caen|se me cae|trabajo en campo|bater[ií]a se me acaba/.test(message)&&!/entiendo|construcci[oó]n|ca[ií]da|resistente|trabajo|bater[ií]a|campo/i.test(answer))findings.push({level:'YELLOW',code:'CONTEXT_NOT_ACKNOWLEDGED',message:'No refleja el contexto/problema explícito del cliente.'});
  const llm=debug.llm;if(llm&&Number(llm.outputTokens??0)>300)findings.push({level:'YELLOW',code:'LLM_OUTPUT_HEAVY',message:`Salida LLM alta: ${llm.outputTokens} tokens.`});if(llm&&Number(llm.totalTokens??0)>1200)findings.push({level:'YELLOW',code:'LLM_TOKEN_HEAVY',message:`Turno LLM costoso: ${llm.totalTokens} tokens.`});if(llm&&Number(llm.durationMs??0)>7000)findings.push({level:'YELLOW',code:'LLM_SLOW',message:`LLM lento: ${llm.durationMs} ms.`});if(debug.telemetry?.delivered===false)findings.push({level:'YELLOW',code:'TELEMETRY_DELIVERY_FAILED',message:`No se pudo persistir telemetría LLM: ${debug.telemetry.error??'sin detalle'}`});if(debug.automation?.delivered===false)findings.push({level:'YELLOW',code:'AUTOMATION_DELIVERY_FAILED',message:`n8n no recibió el evento: ${debug.automation.error??'sin detalle'}`});
  return findings;
}
