...
test('CRM auth validates bearer with Supabase Auth and requires active crm_usuarios membership by user_id',async()=>{
  ...
  const auth=new SupabaseCrmAuth({url:'https://example.supabase.co',serviceRoleKey:'service-secret',authApiKey:'publishable-key',fetcher:fetcher as any});
  ...
  assert.match(calls[1].url,/user_id=eq\.auth-user-1/);
  assert.ok(!calls[1].url.includes('or='));
});

test('CRM auth rejects missing bearer without contacting Supabase',async()=>{
  let calls=0;
  const auth=new SupabaseCrmAuth({url:'https://example.supabase.co',serviceRoleKey:'service-secret',authApiKey:'publishable-key',fetcher:(async()=>{calls+=1;return new Response('{}',{status:200});}) as any});
...
test('CRM membership authority is user_id even if auth response has no email',async()=>{
  const fetcher=async(url:any)=>String(url).endsWith('/auth/v1/user')
    ?new Response(JSON.stringify({id:'auth-user-3',email:null}),{status:200,headers:{'content-type':'application/json'}})
    :new Response(JSON.stringify([{id:'crm-user-3',user_id:'auth-user-3',email:'stored@example.com',nombre:'Admin',rol:'ADMIN',activo:true}]),{status:200,headers:{'content-type':'application/json'}});
  const auth=new SupabaseCrmAuth({url:'https://example.supabase.co',serviceRoleKey:'service-secret',authApiKey:'publishable-key',fetcher:fetcher as any});
  const actor=await auth.authenticate('Bearer user-jwt');
  assert.equal(actor.userId,'auth-user-3');
  assert.equal(actor.email,'stored@example.com');
});
