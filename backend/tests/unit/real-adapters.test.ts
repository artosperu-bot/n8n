...
test('OpenAI adapter sends deterministic evidence through Responses API', async () => {
  ...
  const result = await llm.write({
    ...
    nextBestAction:'ANSWER_ONLY',executableNba:'ANSWER_ONLY',capabilityAction:'ANSWER_ONLY',missingFact:null,decisionImpact:false,
    verifiedFeatures:[],turnCapabilities:{checkPrice:true,scheduleDemo:false},supportedCapabilities:['CHECK_PRICE'],resolvedCurrentIntent:'PRICE',resolvedProduct:'Armor 22',commercialSignals:{activeProduct:'Armor 22'},
    decision: {
  ...
  assert.match(body.input,/"EXECUTABLE_NBA":"ANSWER_ONLY"/);
  assert.match(body.input,/"supportedCapabilities":\["CHECK_PRICE"\]/);
