...
for(const problem of ['reparaciones_repetidas','caidas_frecuentes','autonomia_insuficiente','exposicion_agua_polvo']){
  test(`specific pain ${problem} does not bypass a still-missing implication before recommendation`,()=>{
    const state={
      activeProduct:'Armor 22',useCase:'trabajo',problem,
    ...
    const progression=evaluatePostAnswerCommercialProgression({
      intent:'EVALUATE_USE',currentNba:'ASK_MISSING_FACT',state,resolvedProduct:'Armor 22',verifiedCurrentAnswer:true,
    });
