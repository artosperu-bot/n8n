...
test('recent history provider supplies at most the last three complete turns to semantic planner', async () => {
  ...
  const llm = new RecentHistoryLlmProvider(inner, conversations, 6);
  ...
  assert.deepEqual(history.map(x=>x.content), ['u2','a2','u3','a3','u4','a4']);
});
