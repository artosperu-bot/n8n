...
async function withApp(run:(base:string,calls:any[],worker:any)=>Promise<void>){
  ...
  const crmAuth={async authenticate(value:string|undefined){
    ...
    if(value==='Bearer advisor')return{id:'crm-advisor',userId:'auth-advisor',email:'advisor@s-tech.com.pe',name:'Asesor',role:'ASESOR'};
...
test('automation rule endpoints require auth and only ADMIN can create/toggle',async()=>withApp(async(base,calls)=>{
  ...
  const denied=await fetch(`${base}/api/automations/rules`,{method:'POST',headers:{authorization:'Bearer advisor','content-type':'application/json'},body:JSON.stringify({name:'x',delaySeconds:60,messageTemplate:'Hola'})});
  assert.equal(denied.status,403);
  const created=await fetch(`${base}/api/automations/rules`,{method:'POST',headers:{authorization:'Bearer admin','content-type':'application/json'},body:JSON.stringify({name:'Seguimiento 1h',delaySeconds:3600,messageTemplate:'¿Sigues interesado?',priority:20})});
  ...
  const toggle=await fetch(`${base}/api/automations/rules/r1/disable`,{method:'POST',headers:{authorization:'Bearer admin'}});assert.equal(toggle.status,200);
...
test('ADMIN can list all jobs while ASESOR is limited to assigned sessions',async()=>withApp(async(base,calls)=>{
  const admin=await fetch(`${base}/api/automations/jobs`,{headers:{authorization:'Bearer admin'}});assert.equal(admin.status,200);
  const assigned='whatsapp:assigned';
  const allowed=await fetch(`${base}/api/automations/jobs?sessionId=${encodeURIComponent(assigned)}`,{headers:{authorization:'Bearer advisor'}});assert.equal(allowed.status,200);
  const denied=await fetch(`${base}/api/automations/jobs?sessionId=${encodeURIComponent('whatsapp:other')}`,{headers:{authorization:'Bearer advisor'}});assert.equal(denied.status,403);
...
test('assigned ASESOR can cancel pending jobs for own conversation',async()=>withApp(async(base,calls)=>{
  ...
  const response=await fetch(`${base}/api/automations/cancel`,{method:'POST',headers:{authorization:'Bearer advisor','content-type':'application/json'},body:JSON.stringify({sessionId,reason:'ASESOR_CANCEL'})});
