...
async function withApp(run:(base:string,calls:any[])=>Promise<void>){
  const calls:any[]=[];
  const crmAuth={async authenticate(value:string|undefined){if(!value)throw new Error('CRM_AUTH_REQUIRED');return value===['Bearer','advisor'].join(' ')?{userId:'u2',role:'ASESOR'}:{userId:'u1',role:'ADMIN'};}} as any;
  const automation={
...
test('new rules are created with BOT_MESSAGE_SENT semantics',async()=>withApp(async(base,calls)=>{
  const response=await fetch(`${base}/api/automations/rules`,{method:'POST',headers:jsonAuth('admin'),body:JSON.stringify({name:'Seguimiento',delaySeconds:60,messageTemplate:'Hola',priority:10})});
...
test('ASESOR cannot edit global automation rules',async()=>withApp(async(base)=>{
  const response=await fetch(`${base}/api/automations/rules/r1`,{method:'PATCH',headers:jsonAuth('advisor'),body:JSON.stringify({name:'x',delaySeconds:60,messageTemplate:'x',priority:1})});
