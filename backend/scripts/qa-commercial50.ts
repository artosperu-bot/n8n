...
const result=await runLiveQa({scenarios:commercial50Scenarios,outputDir:'qa-results/commercial50'});
...
for(const scenario of result.report.scenarios){
  ...
  const checks={
    ...
    expectedContextVersion:Number.isFinite(contextVersion)&&contextVersion===expectedTurns,
...
const latest=resolve('qa-results/commercial50/latest');
await mkdir(latest,{recursive:true});
await writeFile(resolve(latest,'commercial50-functional-summary.json'),`${JSON.stringify(functional,null,2)}\n`,'utf8');
await writeFile(resolve(latest,'commercial50-persistence-summary.json'),`${JSON.stringify(persistence,null,2)}\n`,'utf8');
await writeFile(resolve(latest,'commercial50-session-ids.txt'),`${persistenceScenarios.map(x=>`${x.id}\t${x.sessionId}`).join('\n')}\n`,'utf8');

console.log(`COMMERCIAL 50 turns=${COMMERCIAL_50_TURN_COUNT}`);
console.log(`COMMERCIAL 50 FUNCTIONAL GREEN=${functional.summary.green} YELLOW=${functional.summary.yellow} RED=${functional.summary.red}`);
console.log(`COMMERCIAL 50 PERSISTENCE mode=${persistenceMode} GREEN=${persistence.summary.green} RED=${persistence.summary.red} gate=${persistence.status}`);
...
console.log('COMMERCIAL 50 reports: qa-results/commercial50/latest/conversation-report.txt + commercial50-functional-summary.json + commercial50-persistence-summary.json + commercial50-session-ids.txt');
