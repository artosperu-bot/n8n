...
type RunOptions = { baseUrl?:string; scenarios?:QaScenario[]; suite?:QaSuite; fetcher?:typeof fetch; now?:Date; entropy?:string; writeArtifacts?:boolean; outputDir?:string; strict?:boolean; logger?:Logger };
...
export async function runLiveQa(options: RunOptions = {}): Promise<{ report: QaReport; exitCode: number }> {
  ...
  for(const scenario of scenarios){
    ...
    for(let index=0;index<scenario.turns.length;index+=1){
      const turn=scenario.turns[index];const messageId=createMessageId(runId,scenario.id,index+1);const request={sessionId,messageId,message:turn.message};const started=performance.now();let observation:QaTurnObservation;
      try{const response=await fetcher(`${baseUrl}/api/chat`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(request)});const payload=await responseJson(response);observation={httpStatus:response.status,ok:response.ok,request,response:payload,roundTripMs:Math.max(0,Math.round(performance.now()-started))};}
      catch(error){observation={httpStatus:0,ok:false,request,response:{error:error instanceof Error?error.message:String(error)},roundTripMs:Math.max(0,Math.round(performance.now()-started))};}
      const findings=[...evaluateHard(turn,observation),...evaluateCommercial(observation)];const status=statusFromFindings(findings);turnResults.push({turn:index+1,message:turn.message,status,observation,findings});if(!observation.ok||observation.response?.error)break;
    }
