...
const scenarios:QaScenario[]=[{
  ...
  family:'CLOSING',
  title:'Armor 22 repeated-repair pain uses human rugged FAB and survives policy overlay through reservation',
  turns:[
...
const pain=answer(0);
if(!/si ya lo reparaste varias veces/i.test(pain))add('PROMISED_HUMAN_LEAD_MISSING','Debe abrir con la escena humana de reparaciones repetidas, no con una ficha técnica.',pain);
if(!/cada nueva ca[ií]da/i.test(pain)||!/gasto/i.test(pain)||!/quedarte sin celular|quedarte.*sin equipo/i.test(pain))add('PROMISED_PAIN_CONSEQUENCE_MISSING','Debe aterrizar caída → gasto → quedarse sin equipo.',pain);
...
if(!/riesgo|reparaci[oó]n|quedarte|preparado/i.test(pain))add('PROMISED_FAB_BENEFIT_MISSING','Debe conectar la ventaja con el beneficio real del cliente.',pain);
if(!/S\/\s*1399/i.test(pain)||!/disponib/i.test(pain))add('PROMISED_PRICE_STOCK_MISSING','Debe dar S/1399 + disponibilidad juntos.',pain);
if(!/env[ií]o/i.test(pain)||!/recoger|recojo|local/i.test(pain))add('PROMISED_FULFILLMENT_OFFER_MISSING','Debe terminar ofreciendo envío o recojo/local.',pain);
...
if(!/local|direcci[oó]n|ubicaci[oó]n/i.test(policy))add('POLICY_NOT_ANSWERED','Debe responder dónde queda el local usando política institucional.',policy);
if(!/env[ií]o/i.test(policy)||!/recoger|recojo|local/i.test(policy))add('POLICY_DID_NOT_RESUME_FULFILLMENT','Tras responder la política debe retomar envío/recojo.',policy);
