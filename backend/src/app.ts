...
export function createStechApp(options: { env?: EnvLike } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/health') return send(res, 200, { status: 'ok', service: 'stech-backend', modes: { llm: runtime.config.llmMode, erp: runtime.config.erpMode, persistence: runtime.config.persistenceMode, n8n: runtime.config.automationMode } });
