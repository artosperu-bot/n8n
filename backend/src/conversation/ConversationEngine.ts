...
export class ConversationEngine {
  ...
  async processTurn(input: ChatInput): Promise<ChatTurnResult> {
    ...
    let intent: string = budget.preferredIntent ?? baseIntent;
    ...
    else if (intent === 'COMPARE' && reference.mentionedProducts.length === 1 && previous.activeProduct && previous.activeProduct !== reference.mentionedProducts[0]) {
    ...
    if ((intent === 'PRICE' || intent === 'STOCK') && reference.queryTarget) {
      ...
      images = this.deps.erp.getProductImages ? await this.deps.erp.getProductImages(reference.queryTarget, 10) : [];
      ...
      let best = options.at(-1) ?? null;
      let bestScore = -1;
      const needQuery = `${input.message} ${(commercial.priorities ?? []).join(' ')} ${commercial.problem ?? ''}`;
      for (const candidate of options.slice(-4)) {
        const name = canonical(candidate.product) ?? candidate.product;
        const evidence = await this.deps.rag.search(needQuery, name);
        const score = evidence.reduce((n, x) => n + Number(x.score ?? 0), 0);
        if (score > bestScore) { bestScore = score; best = candidate; }
      }
      recommendedProduct = canonical(best?.product) ?? best?.product ?? null;
      if (recommendedProduct) {
        rag = await this.deps.rag.search(needQuery, recommendedProduct);
        deterministicAnswer = `Candidato verificado dentro del presupuesto: ${recommendedProduct}.`;
    ...
    if (intent === 'POLICY' || intent === 'WARRANTY') {
      ...
      rag = await this.deps.rag.search(input.message, reference.queryTarget);
      if (!rag.length) deterministicAnswer = noEvidenceResponse();
    } else if (intent === 'COMPARE') {
      const pair = comparisonProducts.length >= 2 ? comparisonProducts : reference.mentionedProducts;
      for (const product of pair.slice(0, 2)) rag.push(...(await this.deps.rag.search(input.message, product)).slice(0, 2));
      if (!rag.length) deterministicAnswer = noEvidenceResponse();
    }
