    ...
    try {
      ...
      if(requestedUnknown&&intent!=='IMAGE'){
        ...
        if(pair.length<2){answer='¿Qué dos modelos quieres comparar?';route='CLARIFICATION';}
          ...
          writerResult=await safeWrite(this.#deps.llm,{message:input.message,intent,state:{...commercialState,comparisonProducts:pair,recommendedProduct},rag,deterministicAnswer:plan,decision,allowedProducts:writerProducts(pair)},fallback);answer=writerResult.answer;
        }
        ...
        const fallback=noWinner?`${recommendationAlternatives.join(', ')} quedan como alternativas parejas. ¿Qué criterio pesa más para ti?`:subject?`Sobre ${subject}, ese detalle no está especificado.`:(hasDecisionContext?'Aún no hay una opción que destaque con claridad.':'¿Para qué uso principal necesitas el equipo?');
        writerResult=await safeWrite(this.#deps.llm,{message:input.message,intent,state:{...commercialState,recommendedProduct},quote,rag,deterministicAnswer:plan,decision:{...decision,nextBestAction:nba},allowedProducts:writerProducts(recommendationAlternatives)},fallback);answer=writerResult.answer;route=noWinner?'RAG_RECOMMENDATION_NO_WINNER':recommendationTurn&&recommendedProduct?'RAG_RECOMMENDATION':rag.length?'RAG_PRODUCT':'COMMERCIAL_REASONING';
      ...
      else{writerResult=await safeWrite(this.#deps.llm,{message:input.message,intent,state:commercialState,rag:[],deterministicAnswer:'Responde lo actual sin inventar ni repetir preguntas conocidas.',decision:{...decision,nextBestAction:nba},allowedProducts:writerProducts()},'Puedo ayudarte con productos, comparaciones, características, políticas o una compra.');answer=writerResult.answer;route='GENERAL_COMMERCIAL';}
