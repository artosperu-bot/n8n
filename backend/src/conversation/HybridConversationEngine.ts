  ...
  async processTurn(input:ChatInput):Promise<ChatTurnResult>{
    ...
    try {
      ...
      if(requestedUnknown&&intent!=='IMAGE'){
        ...
        const plan=noWinner?`Presenta ${recommendationAlternatives.join(', ')} como alternativas neutrales basadas en catálogo. No declares una ganadora porque ${recommendationTrace?.winnerReason??'falta evidencia diferenciadora'}. Pide un solo criterio útil para decidir. No menciones precio si no fue solicitado.`:subject?`Responde lo actual sobre ${subject} usando únicamente los datos disponibles. Conserva las cifras técnicas exactamente como aparecen; no redondees. Sé corto y humano; si es recomendación, conclusión primero y hasta 3 puntos útiles.${recommendationGuidance} N+1=${nba??'NINGUNO'}. No repitas discovery, no especules y no menciones precio si no fue solicitado.`:`No repitas prioridades ya conocidas. Si existe contexto suficiente, recomienda solo con evidencia; si no hay evidencia suficiente, dilo brevemente. N+1=${nba??'NINGUNO'}.`;
        const fallback=noWinner?`Puedo revisar ${recommendationAlternatives.join(', ')} como alternativas, pero aún no hay evidencia diferenciadora para elegir una ganadora. ¿Qué criterio pesa más para ti?`:subject?`Puedo ayudarte a evaluar ${subject}; prefiero no afirmar una característica que no tenga confirmada.`:(hasDecisionContext?'No pude verificar una opción concreta con suficiente evidencia ahora.':'¿Para qué uso principal necesitas el equipo?');
