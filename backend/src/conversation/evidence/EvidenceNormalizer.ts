...
function connectivityFacts(text:string,row:RagEvidence):VerifiedFact[]{
  const facts:VerifiedFact[]=[];const base=baseFact(row);
  const nfc=text.match(/\bNFC\s*[:=]?\s*(s[ií]|no)\b/i);const fiveG=text.match(/\b(?:conectividad\s+|red\s+|soporte\s+)?5G\s*[:=]?\s*(s[ií]|no)\b/i);const fourG=text.match(/\b(?:red\s+)?4G(?:\s+LTE)?\s*[:=]?\s*(s[ií]|no)\b/i);
  const bluetooth=text.match(/\bversi[oó]n\s+Bluetooth\s*[:=]?\s*([0-9.]+)\b/i);const wifi=text.match(/\best[aá]ndares?\s+Wi-?Fi\s*[:=]?\s*([^.;\n]+)/i);
  if(nfc)facts.push({...base,key:'NFC',value:yesNo(nfc[1])!});if(fiveG)facts.push({...base,key:'5G',value:yesNo(fiveG[1])!});if(fourG)facts.push({...base,key:'4G_LTE',value:yesNo(fourG[1])!});
  if(bluetooth)facts.push({...base,key:'BLUETOOTH_VERSION',value:bluetooth[1]});if(wifi)facts.push({...base,key:'WIFI_STANDARD',value:wifi[1].trim()});
  return facts;
}
function cameraFacts(text:string,row:RagEvidence):VerifiedFact[]{
  ...
  const nightMp=text.match(/\b(?:c[aá]mara\s+)?visi[oó]n\s+nocturna\s*[:=]?\s*(\d+(?:[.,]\d+)?)\s*MP\b/i)??text.match(/\bc[aá]mara\s+nocturna\s*[:=]?\s*(\d+(?:[.,]\d+)?)\s*MP\b/i);
  const nightYes=text.match(/\b(?:c[aá]mara\s+)?visi[oó]n\s+nocturna\s*[:=]?\s*(s[ií]|no)\b/i);
  if(main)facts.push({...base,key:'CAMARA_PRINCIPAL_MP',value:`${decimal(main[1])} MP`});if(front)facts.push({...base,key:'CAMARA_FRONTAL_MP',value:`${decimal(front[1])} MP`});
  if(nightMp){facts.push({...base,key:'VISION_NOCTURNA',value:'Sí'});facts.push({...base,key:'CAMARA_NOCTURNA_MP',value:`${decimal(nightMp[1])} MP`});}else if(nightYes)facts.push({...base,key:'VISION_NOCTURNA',value:yesNo(nightYes[1])!});
  return facts;
}
function thermalFacts(text:string,row:RagEvidence):VerifiedFact[]{
  const facts:VerifiedFact[]=[];const base=baseFact(row);const thermal=text.match(/\bc[aá]mara\s+t[eé]rmica\s*[:=]?\s*(s[ií]|no)\b/i);const resolution=text.match(/\bresoluci[oó]n\s+t[eé]rmica\s*[:=]?\s*(\d+)\s*[x×]\s*(\d+)/i);
  if(thermal)facts.push({...base,key:'CAMARA_TERMICA',value:yesNo(thermal[1])!});if(resolution)facts.push({...base,key:'RESOLUCION_TERMICA',value:`${resolution[1]}×${resolution[2]}`});return facts;
}
function batteryFacts(text:string,row:RagEvidence):VerifiedFact[]{
  const facts:VerifiedFact[]=[];const base=baseFact(row);const capacity=text.match(/\b(?:capacidad(?:\s+de\s+bater[ií]a)?|bater[ií]a)\s*[:=]?\s*(\d+(?:[.,]\d+)?)\s*mAh\b/i);const charge=text.match(/\bcarga(?:\s+cableada)?\s*[:=]?\s*(\d+(?:[.,]\d+)?)\s*W\b/i);
  if(capacity)facts.push({...base,key:'BATERIA_MAH',value:`${decimal(capacity[1])} mAh`});if(charge)facts.push({...base,key:'CARGA_W',value:`${decimal(charge[1])} W`});return facts;
}
function displayFacts(text:string,row:RagEvidence):VerifiedFact[]{
  const facts:VerifiedFact[]=[];const base=baseFact(row);const hz=text.match(/\b(?:frecuencia(?:\s+de\s+refresco)?|refresco)\s*[:=]?\s*(\d+(?:[.,]\d+)?)\s*Hz\b/i);const size=text.match(/\bpantalla(?:\s+de)?\s*[:=]?\s*(\d+(?:[.,]\d+)?)\s*pulgadas\b/i)??text.match(/\b(\d+(?:[.,]\d+)?)\s*pulgadas\b/i);const resolution=text.match(/\b(?:resoluci[oó]n(?:\s+de\s+pantalla)?)\s*[:=]?\s*(\d{3,4})\s*[x×]\s*(\d{3,4})\s*px?\b/i);
  if(hz)facts.push({...base,key:'PANTALLA_HZ',value:`${decimal(hz[1])} Hz`});if(size)facts.push({...base,key:'PANTALLA_TAMANO',value:`${decimal(size[1])} pulgadas`});if(resolution)facts.push({...base,key:'PANTALLA_RESOLUCION',value:`${resolution[1]}×${resolution[2]} px`});return facts;
}
function performanceFacts(text:string,row:RagEvidence):VerifiedFact[]{
  const facts:VerifiedFact[]=[];const base=baseFact(row);const cpu=text.match(/\b(?:procesador|chipset|soc)\s*[:=]?\s*([^.;\n]{3,80})/i);if(cpu)facts.push({...base,key:'PROCESADOR',value:cpu[1].trim()});return facts;
}
...
export function normalizeEvidence(input:{intent:string;quote?:ProductQuote|null;rag?:RagEvidence[]}):VerifiedFact[] {
  ...
  if(q){
    ...
    if(['PRICE','PRICE_AVAILABILITY','STOCK','PRODUCT_INFO','ATTRIBUTE','CAPABILITY'].includes(intent)&&q.stock!=null)facts.push({domain:'SQL',key:'DISPONIBILIDAD',value:q.stock>0?'DISPONIBLE':'NO_DISPONIBLE',productId:q.productRagId??null,source:q.source});
  }
  for(const row of input.rag??[]){
    const raw=String(row.text??'');const atomic=row.domain==='INSTITUTIONAL'?[]:atomicProductFacts(raw,row);facts.push(...atomic);const value=displayText(raw);if(!value)continue;
    const domain=row.domain==='INSTITUTIONAL'||row.source.includes('INSTITUCIONAL')?'INSTITUTIONAL_RAG':'PRODUCT_RAG';const key=String(row.section??row.source.split(':').at(-1)??'EVIDENCIA').toUpperCase();const focusedAttribute=['ATTRIBUTE','CAPABILITY'].includes(intent);
  ...
  const seen=new Set<string>();return facts.filter(f=>{const key=`${f.domain}|${f.key}|${f.value}`;if(seen.has(key))return false;seen.add(key);return true;}).slice(0,24);
}
