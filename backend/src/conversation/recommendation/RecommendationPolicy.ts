...
const PRIORITY_MAP:Record<string,string>={
  ...
  redes:'REDES',pantalla:'PANTALLA',fisico:'FISICO',precio:'PRECIO',
};
...
function metricsFor(rows:RagEvidence[],criterion:string):Metrics{
  ...
  if(criterion==='POSICIONAMIENTO')return{
    systems:['gps','glonass','galileo','beidou'].reduce((n,key)=>n+(new RegExp(`\\b${key}\\b`).test(text)?1:0),0),
  };
...
function criteriaFrom(context:RecommendationContext):string[]{
  const explicit=(context.priorities??[]).map(p=>PRIORITY_MAP[fold(p)]).filter(Boolean);
  ...
  if(/caida|durabilidad/.test(problem))inferred.push('RESISTENCIA');
  if(/autonomia/.test(problem))inferred.push('BATERIA');
...
function weightedRaw(metrics:Metrics,criterion:string):number|null{
  ...
  if(criterion==='MEMORIA')return (metrics.ram??0)*20+(metrics.storage??0);
  if(criterion==='CAMARA')return (metrics.mainMp??0)+(metrics.nightMp??0)*0.6;
...
export function rankRecommendations(candidates:RecommendationCandidate[],context:RecommendationContext={}):RankedRecommendation[]{
  ...
  const ranked:RankedRecommendation[]=rows.map(row=>{
  ...
  ranked.sort((a,b)=>b.score-a.score||b.confidence-a.confidence||Number(a.quote.price??Infinity)-Number(b.quote.price??Infinity));
  if(ranked.length>1){
