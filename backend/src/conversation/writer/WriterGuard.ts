...
function questionConsumesMissingFact(input:LlmWriteInput,answer:string):boolean{
  ...
  return missing.includes('uso')?/\buso\b|para que/.test(question)
    :/impacto|implicacion|consecuencia/.test(missing)?/impacto|afecta|que (?:te )?(?:genera|ocasiona|provoca)|consecuencia|pierdes?|interrump|parar|detener/.test(question)
    :missing.includes('problema')?/\bproblema\b|que te pasa|que te complica|que falla|que ocurre/.test(question)
    :/prioridad|criterio/.test(missing)?/prioridad|priorizas|importa|criterio|pesa mas|mas importante/.test(question)
...
function acknowledgeKnownContext(input:LlmWriteInput,answer:string):string{
  ...
  const acknowledgement=useCase?`Perfecto, si es para ${humanizeCommercialFact(useCase)},`:problem?`Entiendo; entonces el punto importante es ${humanizeCommercialFact(problem)}.`:priorities[0]?`Perfecto, tomo ${humanizeCommercialFact(priorities[0])} como prioridad.`:'';
...
function executeNba(input:LlmWriteInput,answer:string):string {
  ...
  if(action==='ASK_MISSING_FACT'){
    ...
    if(String(input.intent??'').toUpperCase()==='HANDLE_PRICE_OBJECTION'&&!/claro|se sale|ajust|c[oó]mod|alto|caro/i.test(response))response=`Claro; busquemos una opción que se ajuste mejor. ${response}`.trim();
    ...
    const question=missing.includes('uso')?'¿Para qué uso principal lo necesitas?'
      :missing.includes('problema')?'¿Qué es lo que más te complica con tu equipo actual?'
      :/impacto|implicacion|consecuencia/.test(missing)?'¿Y cuando pasa eso, cómo te afecta en el trabajo?'
      :/presupuesto|tope/.test(missing)?'¿Cuál es tu presupuesto máximo?'
      :/prioridad|criterio/.test(missing)?'¿Qué pesa más para ti al elegirlo?'
      :/modelo|producto/.test(missing)?'¿Qué modelo estás evaluando?'
      :'¿Qué dato te ayudaría más a decidir?';
...
function guardGeneratedAnswer(input:LlmWriteInput,answer:string):string|null {
  ...
  const executableNbaForPrice=String(input.executableNba??input.finalExecutableNba??input.nextBestAction??input.decision?.nextBestAction??'').toUpperCase();
  const sellerLedPrice=executableNbaForPrice==='SOFT_CLOSE'
    && intent!=='STOCK'
    && input.commercialResponsePlan?.closePurpose==='FULFILLMENT'
    && input.quote?.price!=null
    && input.quote?.stock!=null
    && moneySupported(input,answer);
  if(!explicitPrice&&!evidenceBoundPrice&&!institutionalPrice&&!sellerLedPrice&&/\bS\/\s*\d/i.test(answer))return 'UNSOLICITED_PRICE';
...
function internalFallback(answer:string):boolean{return /\b(?:SOFT_CLOSE_TO_STOCK|SOFT_CLOSE|ANSWER_ONLY|RELATED_VALUE|ASK_MISSING_FACT|ASK_USE_CASE|ASK_PROBLEM|ASK_IMPLICATION|ASK_PRIORITY|ASK_BUDGET|OFFER_ALTERNATIVE|COLLECT_RESERVATION_DATA|RESERVATION_DATA_COLLECTION|EXECUTE_RESERVATION|ASSISTED_HANDOFF|RECOMMEND_WITHIN_BUDGET|N\+1)\b|\bcompara\b[^.]{0,90}\bde forma sim[eé]trica\b/i.test(answer);}
