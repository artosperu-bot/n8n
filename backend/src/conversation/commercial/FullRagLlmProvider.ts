...
function naturalSalesPlan(input:LlmWriteInput):string{
  ...
  const style=[
    'FULL_RAG_STYLE:','Habla como asesor comercial humano, no como ficha técnica ni evaluador.',
...
export class FullRagLlmProvider implements LlmProvider{
  ...
  async write(input:LlmWriteInput):Promise<LlmResult>{
    ...
    // A cold comparison should never depend on the LLM inventing/choosing numbers.
    // Compose it directly from the two product RAG envelopes; contextual follow-ups
    // still go through the writer so it can turn the differences into advice.
    if(String(enriched.intent??'').toUpperCase()==='COMPARE'&&!meaningfulCustomerContext(enriched)){
      const comparison=buildColdRagComparison(enriched.rag??[]);
      if(comparison)return deterministicResult(comparison,'full-rag-compare-v1');
    }
