...
function parseRow(store:MutableStore,row:RagEvidence):void{
  ...
  store.display.size=assign(store.display.size,first(text,/(?:tama[nñ]o\s+de\s+pantalla|pantalla(?:\s+de)?)\s*[:=]?\s*([0-9.,]+\s*pulgadas)/i)??first(text,/([0-9.,]+\s*pulgadas)/i));store.display.refresh=assign(store.display.refresh,first(text,/(?:frecuencia\s+de\s+(?:refresco|pantalla)|frecuencia\s+de\s+pantalla|refresco|frecuencia)\s*[:=]?\s*([0-9.,]+\s*Hz)/i));const displayRes=text.match(/(?:resoluci[oó]n(?:\s+de\s+pantalla)?)\s*[:=]?\s*(\d{3,4})\s*[x×]\s*(\d{3,4})/i);if(!store.display.resolution&&displayRes)store.display.resolution=`${displayRes[1]}×${displayRes[2]}`;
