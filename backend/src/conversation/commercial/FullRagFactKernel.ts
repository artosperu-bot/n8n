...
function bool(text:string,label:RegExp):boolean|null{const m=text.match(new RegExp(`${label.source}\\s*[:=]?\\s*(s[ií]|no|yes|true|false)${PUNCT}`,'i'));if(!m)return null;return /^(?:s[ií]|yes|true)$/i.test(m[1]);}
...
function parseRow(store:MutableStore,row:RagEvidence):void{
  ...
  store.resistance.ip68=store.resistance.ip68??bool(text,/(?:certificaci[oó]n\s+)?IP68/);store.resistance.ip69k=store.resistance.ip69k??bool(text,/(?:certificaci[oó]n\s+)?IP69K?/);store.resistance.mil810h=store.resistance.mil810h??bool(text,/MIL-STD-810H/);
  ...
  store.camera.nightMp=assign(store.camera.nightMp,first(text,/(?:c[aá]mara\s+de\s+)?visi[oó]n\s+nocturna\s*[:=]?\s*([0-9.,]+\s*MP)/i)??first(text,/c[aá]mara\s+nocturna\s*[:=]?\s*([0-9.,]+\s*MP)/i));store.camera.nightVision=store.camera.nightVision??(store.camera.nightMp?true:bool(text,/(?:c[aá]mara\s+)?visi[oó]n\s+nocturna/));store.camera.nightSensor=assign(store.camera.nightSensor,first(text,/sensor\s+c[aá]mara\s+nocturna\s*[:=]?\s*([^.;\n]+)/i));store.camera.videoMax=assign(store.camera.videoMax,first(text,/resoluci[oó]n\s+m[aá]xima\s+de\s+video\s*[:=]?\s*([^.;\n]+)/i));
  store.connectivity.nfc=store.connectivity.nfc??bool(text,/\bNFC\b/);store.connectivity.googlePay=store.connectivity.googlePay??bool(text,/Google\s+Pay/);store.connectivity.bluetooth=assign(store.connectivity.bluetooth,first(text,/versi[oó]n\s+Bluetooth\s*[:=]?\s*([^.;\n]+)/i));store.connectivity.wifiStandard=assign(store.connectivity.wifiStandard,first(text,/(?:est[aá]ndares?\s+)?Wi-?Fi\s*[:=]?\s*(802\.11[^.;\n]+)/i));store.connectivity.wifi5=store.connectivity.wifi5??bool(text,/Wi-?Fi\s+5\s*GHz/);store.connectivity.infrared=store.connectivity.infrared??bool(text,/(?:puerto\s+)?infrarrojo/);store.connectivity.jack35=store.connectivity.jack35??bool(text,/(?:jack|conector|entrada)\s*(?:de\s*)?3[.,]5\s*mm/);
  store.network.fiveG=store.network.fiveG??bool(text,/(?:conectividad\s+|red\s+|soporte\s+)?5G/);store.network.fourG=store.network.fourG??bool(text,/(?:red\s+)?4G(?:\s+LTE)?/);if(store.network.fourG==null&&/\b4G\s+LTE\b/i.test(text))store.network.fourG=true;
  store.sim.count=assign(store.sim.count,first(text,/cantidad\s+de\s+SIM\s*[:=]?\s*([^.;\n]+)/i));store.sim.type=assign(store.sim.type,first(text,/tipo\s+de\s+SIM\s*[:=]?\s*([^.;\n]+)/i));store.sim.dual4g=store.sim.dual4g??bool(text,/Dual\s+4G/);store.sim.slots=assign(store.sim.slots,first(text,/cantidad\s+total\s+de\s+ranuras\s*[:=]?\s*([^.;\n]+)/i));
  ...
  store.thermal.camera=store.thermal.camera??bool(text,/c[aá]mara\s+t[eé]rmica/);store.thermal.frequency=assign(store.thermal.frequency,first(text,/frecuencia\s+t[eé]rmica\s*[:=]?\s*([0-9.,]+\s*Hz)/i));const thermalRes=text.match(/resoluci[oó]n\s+t[eé]rmica\s*[:=]?\s*(\d+)\s*[x×]\s*(\d+)/i);if(!store.thermal.resolution&&thermalRes)store.thermal.resolution=`${thermalRes[1]}×${thermalRes[2]}`;const tx=first(text,/resoluci[oó]n\s+t[eé]rmica\s+horizontal\s*[:=]?\s*([0-9]+)\s*px/i),ty=first(text,/resoluci[oó]n\s+t[eé]rmica\s+vertical\s*[:=]?\s*([0-9]+)\s*px/i);if(!store.thermal.resolution&&tx&&ty)store.thermal.resolution=`${tx}×${ty}`;store.thermal.minTemp=assign(store.thermal.minTemp,first(text,/temperatura\s+m[ií]nima\s+t[eé]rmica\s*[:=]?\s*(-?[0-9.,]+\s*°?C)/i));store.thermal.maxTemp=assign(store.thermal.maxTemp,first(text,/temperatura\s+m[aá]xima\s+t[eé]rmica\s*[:=]?\s*(-?[0-9.,]+\s*°?C)/i));store.thermal.app=assign(store.thermal.app,first(text,/aplicaci[oó]n\s+t[eé]rmica\s*[:=]?\s*([^.;\n]+)/i));
