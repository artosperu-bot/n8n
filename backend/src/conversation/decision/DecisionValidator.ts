...
const NBA_ALIASES: Record<string,string> = {
  ASK_NEED:'ASK_MISSING_FACT', ASK_USE:'ASK_MISSING_FACT', ASK_BUDGET:'ASK_MISSING_FACT', ASK_PRIORITY:'ASK_MISSING_FACT',
  DISCOVER_ONE_FACT:'ASK_MISSING_FACT', CLARIFY_OR_HANDOFF:'ASK_MISSING_FACT', CONTINUE_BY_NEED:'ANSWER_ONLY', CONNECT_TO_USE:'ANSWER_ONLY',
  WAIT_FOR_NEXT_QUESTION:'ANSWER_ONLY', WAIT_FOR_PRODUCT_QUESTION:'ANSWER_ONLY', RETURN_TO_PRODUCT:'ANSWER_ONLY', ADVANCE_IF_INTEREST:'SOFT_CLOSE',
  RECOMMEND_BY_NEED:'RECOMMEND', RECOMMEND_BY_PRIORITY:'RECOMMEND', EXPLAIN_FIT:'SOFT_CLOSE', ADDRESS_OBJECTION:'OFFER_ALTERNATIVE',
  OFFER_ALTERNATIVES:'OFFER_ALTERNATIVE', GUIDE_SELECTION:'OFFER_ALTERNATIVE',
};
...
function editDistance(a:string,b:string):number{const rows=a.length+1,cols=b.length+1;const d=Array.from({length:rows},()=>Array<number>(cols).fill(0));for(let i=0;i<rows;i++)d[i][0]=i;for(let j=0;j<cols;j++)d[0][j]=j;for(let i=1;i<rows;i++)for(let j=1;j<cols;j++){const cost=a[i-1]===b[j-1]?0:1;d[i][j]=Math.min(d[i-1][j]+1,d[i][j-1]+1,d[i-1][j-1]+cost);if(i>1&&j>1&&a[i-1]===b[j-2]&&a[i-2]===b[j-1])d[i][j]=Math.min(d[i][j],d[i-2][j-2]+1);}return d[a.length][b.length];}
function fuzzyCanonical(raw:string,universe:string[]):string|null{const parts=fold(raw).match(/[a-z0-9]+/g)??[];const model=parts.find(x=>/\d/.test(x));if(!model)return null;const scored=universe.map(product=>{const p=fold(product).match(/[a-z0-9]+/g)??[];const modelMatch=p.includes(model)?3:0;const family=p.filter(x=>!/[0-9]/.test(x)&&x.length>=4);const familyMatch=family.some(word=>parts.some(q=>q===word||(q.length>=4&&editDistance(q,word)<=1)))?1:0;return{product,score:modelMatch+familyMatch};}).filter(x=>x.score>=3).sort((a,b)=>b.score-a.score);if(!scored.length)return null;if(scored[1]&&scored[1].score===scored[0].score)return null;return scored[0].product;}
...
function canonicalNba(value:string|null|undefined):string|null{const v=String(value??'').trim().toUpperCase();const normalized=NBA_ALIASES[v]??v;return NBAS.has(normalized)?normalized:null;}
...
function canonicalReference(value:string|null|undefined,fallback:string|null|undefined):string|null{const v=String(value??'').trim().toUpperCase();const aliases:Record<string,string>={SELECTION:'SELECTION_REFERENT',NAMED:'NAMED_QUERY_TARGET',RECOMMENDED:'RECOMMENDED_REFERENT',OTHER:'COMPARISON_ALTERNATIVE'};const normalized=aliases[v]??v;const allowed=new Set(['ACTIVE_PRODUCT_FALLBACK','UNKNOWN_PRODUCT_MENTION','EXPLICIT_PRODUCT_SWITCH','MULTI_PRODUCT_MENTION','NAMED_QUERY_TARGET','SELECTION_REFERENT','RECOMMENDED_REFERENT','COMPARISON_ALTERNATIVE','RECOMMENDED_FALLBACK']);if(allowed.has(normalized))return normalized;const fb=String(fallback??'').trim().toUpperCase();return allowed.has(fb)?fb:null;}
...
export function validateTurnDecision(decision:TurnDecision,state:ConversationState,catalogCandidates:string[]=[],fallbackDecision?:TurnDecision):TurnDecision{
  ...
  let primaryIntent=canonicalIntent(decision.primaryIntent)??fallbackIntent??'OTHER';
  ...
  let authorityReason=knownDecisionTarget?'CANONICAL_PLANNER_TARGET':'CONTEXT_FALLBACK';
  ...
  const proposedNba=canonicalNba(decision.nextBestAction),fallbackNba=canonicalNba(fallbackDecision?.nextBestAction);
  ...
  const normalizedMentions=currentTurnTarget&&!sameStringListContains(currentMentions,currentTurnTarget)&&uniqueNewCatalogTarget&&fold(currentTurnTarget)===fold(uniqueNewCatalogTarget)?unique([...currentMentions,currentTurnTarget]):currentMentions;

  if(state.sessionId&&(uniqueNewCatalogTarget||knownDecisionTarget&&state.activeProduct&&!sameProduct(knownDecisionTarget,state.activeProduct)||referenceType!=='ACTIVE_PRODUCT_FALLBACK')){
    console.log(JSON.stringify({event:'STECH_REFERENCE_TRACE',sessionId:state.sessionId,activeBefore:state.activeProduct??null,selectedBefore:state.selectedProduct??null,recommendedBefore:state.recommendedProduct??null,plannerTarget:decision.targetProduct??null,deterministicTarget:fallbackDecision?.targetProduct??null,plannerReference:decision.referenceType??null,deterministicReference:fallbackDecision?.referenceType??null,catalogCandidates,newCatalogCandidates,currentMentions:normalizedMentions,finalTarget:targetProduct??null,finalReference:referenceType??null,selectedProduct:selectedProduct??null,explicitSwitch,authorityReason}));
  }
