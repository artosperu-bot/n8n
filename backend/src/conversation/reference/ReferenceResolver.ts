...
export function resolveReference(message:string,state:ReferenceState):ReferenceResolution{
  ...
  let reason='ACTIVE_PRODUCT_FALLBACK';if(unknownNamedProduct)reason='UNKNOWN_PRODUCT_MENTION';else if(named)reason=namedSwitch?'EXPLICIT_PRODUCT_SWITCH':mentionedProducts.length>1?'MULTI_PRODUCT_MENTION':'NAMED_QUERY_TARGET';else if(selectionRef)reason='SELECTION_REFERENT';else if(recommendedRef)reason='RECOMMENDED_REFERENT';else if(otherRef)reason='COMPARISON_ALTERNATIVE';else if(!state.activeProduct&&recommended)reason='RECOMMENDED_FALLBACK';
