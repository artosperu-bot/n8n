...
export function canonicalProductName(value: string | null | undefined): string | null {
  ...
  const t = fold(value);
  return PRODUCTS.find(p => t.includes(fold(p))) ?? null;
}

function namedProducts(message: string): string[] {
  const t = fold(message);
  return PRODUCTS.filter(p => t.includes(fold(p)));
}

function comparisonAlternative(state: ReferenceState): string | null {
  const pair = state.comparisonProducts ?? [];
  if (pair.length < 2) return null;
  const active = state.activeProduct ?? null;
  return pair.find(p => p !== active) ?? pair[1] ?? null;
}

export function resolveReference(message: string, state: ReferenceState): ReferenceResolution {
  const t = fold(message);
  const mentionedProducts = namedProducts(message);
  const named = mentionedProducts[0] ?? null;
  ...
  const referentialTarget = selectionRef
    ? recommended ?? state.activeProduct ?? null
    : recommendedRef
      ? recommended
      : otherRef
  ...
  const queryTarget = product ?? state.activeProduct ?? null;
  let nextActiveProduct = explicitSwitch ? queryTarget : (state.activeProduct ?? null);
  if (!nextActiveProduct && mentionedProducts.length) nextActiveProduct = mentionedProducts[0];

  let reason = 'ACTIVE_PRODUCT_FALLBACK';
  if (named) reason = namedSwitch ? 'EXPLICIT_PRODUCT_SWITCH' : mentionedProducts.length > 1 ? 'MULTI_PRODUCT_MENTION' : 'NAMED_QUERY_TARGET';
  ...
  return { queryTarget, explicitSwitch, nextActiveProduct, reason, mentionedProducts };
}
