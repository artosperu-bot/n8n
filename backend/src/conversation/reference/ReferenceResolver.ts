...
function directNamedProducts(message: string): string[] {
...
export function resolveReference(message: string, state: ReferenceState): ReferenceResolution {
  const t = fold(message);
  const directProducts = directNamedProducts(message);
  ...
  const armorMention = /\barmor\s+[a-z0-9][a-z0-9 ]{0,20}\b/.test(t);
  const unknownNamedProduct = armorMention && directProducts.length === 0;
  ...
  if (!nextActiveProduct && directProducts.length) nextActiveProduct = directProducts[0];
