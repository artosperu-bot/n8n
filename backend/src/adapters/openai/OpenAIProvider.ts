...
export class OpenAIProvider implements LlmProvider {
  ...
  async write(input: LlmWriteInput): Promise<LlmResult> {
    ...
    const json = await this.#responses({
      ...
      instructions,
      input: `CLIENTE:\n${input.message}\n\nCONTRATO_COMERCIAL:\n${JSON.stringify({resolvedCurrentIntent:input.resolvedCurrentIntent,commercialStage:input.commercialStage,commercialSignals:input.commercialSignals,knownFacts:input.knownFacts,missingFacts:input.missingFacts,missingFact:input.missingFact,decisionImpact:input.decisionImpact,verifiedFeatures:input.verifiedFeatures,resolvedProduct:input.resolvedProduct,recommendedProduct:input.recommendedProduct,previousRecommendedProduct:input.previousRecommendedProduct,recommendationChanged:input.recommendationChanged,recommendationChangeReason:input.recommendationChangeReason,levelOfInterest:input.levelOfInterest,attribute:input.attribute,implications:input.implications,pendingQuestion:input.pendingQuestion,pendingAction:input.pendingAction,supportedCapabilities:input.supportedCapabilities,EXECUTABLE_NBA:input.executableNba,capabilityAction:input.capabilityAction,alternatives:input.alternatives,customerContext:input.customerContext})}\n\nCONTEXTO_COMERCIAL:\n${JSON.stringify(this.#compactState(input.state))}\n\nPLAN_DE_RESPUESTA:\n${input.deterministicAnswer ?? 'SIN_PLAN'}\n\nVERIFICADOS:\n${evidence || 'SIN_DATO'}`,
    });
    return {
