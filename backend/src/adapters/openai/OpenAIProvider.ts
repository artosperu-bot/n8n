...
export class OpenAIProvider implements LlmProvider {
  ...
  async write(input: LlmWriteInput): Promise<LlmResult> {
    ...
    const instructions = [
      ...
      'Ejecuta exactamente ACCION_COMERCIAL.nextBestAction: ANSWER_ONLY significa responder y terminar sin pregunta; RELATED_VALUE significa redactar solo la respuesta principal y no inventar otro CTA porque la continuación segura de relatedNextValue se añade en la capa determinística; RECOMMEND exige nombrar claramente la opción recomendada; ASK_MISSING_FACT exige preguntar solo ACCION_COMERCIAL.missingFact; OFFER_ALTERNATIVE exige ofrecer una alternativa; SOFT_CLOSE permite un único siguiente paso contextual, sin presión ni volver a discovery.',
    ...
    const json = await this.#responses({
      ...
      instructions,
      input: `CLIENTE:\n${input.message}\n\nCONTRATO_COMERCIAL:\n${JSON.stringify({resolvedCurrentIntent:input.resolvedCurrentIntent,commercialStage:input.commercialStage,commercialSignals:input.commercialSignals,knownFacts:input.knownFacts,missingFacts:input.missingFacts,missingFact:input.missingFact,decisionImpact:input.decisionImpact,verifiedFeatures:input.verifiedFeatures,relatedNextValue:input.relatedNextValue,resolvedProduct:input.resolvedProduct,recommendedProduct:input.recommendedProduct,previousRecommendedProduct:input.previousRecommendedProduct,recommendationChanged:input.recommendationChanged,recommendationChangeReason:input.recommendationChangeReason,levelOfInterest:input.levelOfInterest,attribute:input.attribute,implications:input.implications,pendingQuestion:input.pendingQuestion,pendingAction:input.pendingAction,supportedCapabilities:input.supportedCapabilities,EXECUTABLE_NBA:input.executableNba,capabilityAction:input.capabilityAction,alternatives:input.alternatives,customerContext:input.customerContext})}\n\nCONTEXTO_COMERCIAL:\n${JSON.stringify(this.#compactState(input.state))}\n\nPLAN_DE_RESPUESTA:\n${input.deterministicAnswer ?? 'SIN_PLAN'}\n\nVERIFICADOS:\n${evidence || 'SIN_DATO'}`,
    });
    return {
